...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  function scheduleRefresh(token: string) {
    try {
      ...
      const msLeft = exp * 1000 - Date.now() - 60_000
  ...
  const silentRefresh = useCallback(async () => {
    ...
    try {
      ...
      const email = decodeEmail(data.access)
  ...
  useEffect(() => {
    ...
    if (access) {
      ...
      const email = decodeEmail(access)
      if (email) setUser({ email })
  ...
  async function login(email: string, password: string) {
    ...
    const decodedEmail = decodeEmail(tokens.access)
    setUser({ email: decodedEmail ?? email })
  ...
  async function register(email: string, password: string, password2: string) {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}))
      const msg = err.email?.[0] ?? err.password?.[0] ?? err.non_field_errors?.[0] ?? 'Registration failed'
      throw new Error(msg)
    }
