...
const TABS: { id: AnalysisTab; label: string; shortLabel: string; icon: React.ReactNode; placeholder: string }[] = [
  {
    id: 'text',
    label: 'Text / SMS / Email',
    shortLabel: 'Text',
    icon: <MessageSquare size={15} />,
    placeholder: 'Paste a suspicious SMS, email body, or any text here…\n\nExamples:\n• "Congratulations! You won ₦5,000,000…"\n• "Dear customer, your GTBank account has been suspended…"\n• "Your OTP is 847291. Never share this…"',
  },
  {
    id: 'url',
    label: 'URL',
    shortLabel: 'URL',
    icon: <Link2 size={15} />,
    placeholder: 'Paste a suspicious link, e.g. https://gtbank-verify-now.com/login',
  },
  {
    id: 'image',
    label: 'Screenshot',
...
export function AnalyserPanel() {
  ...
  if (result) {
    // fingerprint comes straight from the backend now — null when is_scam is false
    return <ResultCard result={result} fingerprint={result.fingerprint ?? undefined} onReset={reset} />
  }
      ...
      <div className="flex border-b border-white/6">
        {TABS.map(t => (
          <button
            ...
            className={cn(
              'relative flex-1 flex items-center justify-center gap-1.5 py-4 text-xs font-semibold transition-all duration-200',
              tab === t.id ? 'opacity-100' : 'opacity-35 hover:opacity-60'
            )}
            ...
            >
                ...
                <div className="space-y-3">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="https://suspicious-link.com/verify-now"
                  ...
                  <p className="text-xs opacity-30 pl-1">
                    We fetch the page content and run a full AI analysis — not just the URL pattern.
                  </p>
                </div>
              )}
              ...
              {tab === 'image' && (
                <div className="space-y-3">
                  ...
                  <p className="text-xs opacity-30 pl-1">
                    Upload a screenshot of a suspicious message. Gemini will extract and analyse all visible text.
                  </p>
                </div>
              )}
              ...
              {tab === 'audio' && (
                <div className="space-y-3">
                  ...
                  <p className="text-xs opacity-30 pl-1">
                    Upload a voice note or record live. Gemini transcribes and checks for spoken scam tactics, fake bank numbers, and impersonation.
                  </p>
                </div>
              )}
              ...
              {error && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="flex items-start gap-2.5 p-3.5 rounded-xl text-sm text-red-400"
              ...
              <button
                ...
                className={cn(
                  'w-full py-4 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2',
                  'transition-all duration-200',
                  canSubmit() ? 'text-white hover:scale-[1.02] active:scale-[0.98]' : 'opacity-30 cursor-not-allowed'
                )}
                style={{
                  fontFamily: 'Space Grotesk, sans-serif',
                  background: canSubmit() ? 'linear-gradient(135deg, #00D4FF, #7C3AED)' : 'rgba(255,255,255,0.08)',
                  boxShadow: canSubmit() ? '0 0 32px rgba(0,212,255,0.3)' : 'none',
                }}
              >
                <Send size={15} />
                Scan for Scams
